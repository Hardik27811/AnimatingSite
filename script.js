// // let crsr=document.querySelector('#cursor');
// // let blur=document.querySelector("#cursor-blur");
// // document.addEventListener('mousemove',function(dets){
// //     crsr.style.left=dets.x-10+'px';
// //     crsr.style.top=dets.y-10+"px";
// //     blur.style.left=dets.x-250+"px";
// //     blur.style.top=dets.y-250+"px";
// // })

// var crsr = document.querySelector("#cursor");
// var blur = document.querySelector("#cursor-blur");

// document.addEventListener("mousemove", function (dets) {
//   crsr.style.left = dets.x + "px";
//   crsr.style.top = dets.y + "px";
//   blur.style.left = dets.x - 250 + "px";
//   blur.style.top = dets.y - 250 + "px";
// });

// var h4all=document.querySelectorAll('#nav h4')
// h4all.forEach(function(elem){
//     elem.addEventListener('mouseenter',function(){
//         crsr.style.scale=4;
//         crsr.style.border="0.1px solid #fff";
//         crsr.style.backgroundColor="transparent";
//     })
//      elem.addEventListener('mouseleave',function(){
//         crsr.style.scale=1;
//         crsr.style.border="0 px solid #95C11E";
//         crsr.style.backgroundColor="#95C11E";
//     })
// })
// let arrow = document.querySelector("#arrow");

// arrow.addEventListener("mouseenter", function () {
//   crsr.style.scale = 4;
//   crsr.style.border = "0.1px solid #fff";
//   crsr.style.backgroundColor = "transparent";
// });

// arrow.addEventListener("mouseleave", function () {
//   crsr.style.scale = 1;
//   crsr.style.border = "0px solid #95C11E";
//   crsr.style.backgroundColor = "#95C11E";
// });



// gsap.to("#nav",{
//     backgroundColor:"#000",
//     height:"110px",
//     duration:0.5,
//     scrollTrigger:{
//         trigger:"#nav",
//         scroller:"body",
//         // markers:true,
//         start:"top -10%",
//         end: "top -11%",
//         scrub:1
//     }
// })

// gsap.to("#main", {
//   backgroundColor: "#000",
//   scrollTrigger: {
//     trigger: "#main",
//     scroller: "body",
//     // markers: true,
//     start: "top -25%",
//     end: "top -70%",
//     scrub: 2,
//   },
// });


// gsap.from("#about-us img,#about-us-in",{
//     y:50,
//     opacity:0,
//     duration:1,
//     scrollTrigger:{
//         trigger:"#about-us",
//         scroller:"body",
//         // markers:true,
//         start:"top 70%",
//         end:"top 65%",
//         scrub: 1
//     }
// })
// gsap.from(".cards",{
//     scale:0.8,
//     opacity:0,
//     duration:1,
//     scrollTrigger:{
//         trigger:".cards",
//         scroller:"body",
//         // markers:true,
//         start:"top 70%",
//         stop:"top 65%",
//         scrub:1
//     }
// })
// gsap.from("#colon1",{
//     y:-70,
//     x:-70,
//     scrollTrigger:{
//         trigger:"#colon1",
//         scroller:"body",
//         // markers:true,
//         start:"top 55%",
//         end:"top 45%",
//         scrub:4
//     }
// })
// gsap.from("#colon2",{
//     y:70,
//     x:70,
//     scrollTrigger:{
//         trigger:"#colon1",
//         scroller:"body",
//         // markers:true,
//         start:"top 55%",
//         end:"top 45%",
//         scrub:4
//     }
// })
// gsap.from("#page4 h1", {
//   y: 50,
//   opacity: 0,
//   duration: 1,
//   scrollTrigger: {
//     trigger: "#page4",
//     scroller: "body",
//     start: "top 75%",
//     end: "top 50%",
//     scrub: 2
//     // markers: true
//   }
// });



// === Custom cursor ===
var crsr = document.querySelector("#cursor");
var blur = document.querySelector("#cursor-blur");

// Follow mouse position
document.addEventListener("mousemove", function (dets) {
  crsr.style.left = dets.x + "px";
  crsr.style.top = dets.y + "px";
  blur.style.left = dets.x - 250 + "px";
  blur.style.top = dets.y - 250 + "px";
});

// === Hover effect on nav items ===
var h4all = document.querySelectorAll("#nav h4");
h4all.forEach(function (elem) {
  elem.addEventListener("mouseenter", function () {
    crsr.style.scale = 4;
    crsr.style.border = "0.1px solid #fff";
    crsr.style.backgroundColor = "transparent";
  });
  elem.addEventListener("mouseleave", function () {
    crsr.style.scale = 1;
    crsr.style.border = "0px solid #95C11E";
    crsr.style.backgroundColor = "#95C11E";
  });
});

// === Arrow hover effect ===
let arrow = document.querySelector("#arrow");

arrow.addEventListener("mouseenter", function () {
  crsr.style.scale = 4;
  crsr.style.border = "0.1px solid #fff";
  crsr.style.backgroundColor = "transparent";
});

arrow.addEventListener("mouseleave", function () {
  crsr.style.scale = 1;
  crsr.style.border = "0px solid #95C11E";
  crsr.style.backgroundColor = "#95C11E";
});

// === NEW: Click + hover to scroll ===
let scrolling = false;

// Start scrolling when mouse is down and on arrow
arrow.addEventListener("mousedown", function () {
  scrolling = true;
  scrollContinuously();
});

// Stop scrolling when mouse up or leaves arrow
arrow.addEventListener("mouseup", function () {
  scrolling = false;
});

arrow.addEventListener("mouseleave", function () {
  scrolling = false;
});

// Actual scroll function
function scrollContinuously() {
  if (!scrolling) return;
  window.scrollBy(0, 5); // 5px per frame
  requestAnimationFrame(scrollContinuously);
}

// === GSAP nav scroll effect ===
gsap.to("#nav", {
  backgroundColor: "#000",
  height: "110px",
  duration: 0.5,
  scrollTrigger: {
    trigger: "#nav",
    scroller: "body",
    start: "top -10%",
    end: "top -11%",
    scrub: 1,
  },
});

// === GSAP main background scroll ===
gsap.to("#main", {
  backgroundColor: "#000",
  scrollTrigger: {
    trigger: "#main",
    scroller: "body",
    start: "top -25%",
    end: "top -70%",
    scrub: 2,
  },
});

// === GSAP about us section ===
gsap.from("#about-us img,#about-us-in", {
  y: 50,
  opacity: 0,
  duration: 1,
  scrollTrigger: {
    trigger: "#about-us",
    scroller: "body",
    start: "top 70%",
    end: "top 65%",
    scrub: 1,
  },
});

// === GSAP cards ===
gsap.from(".cards", {
  scale: 0.8,
  opacity: 0,
  duration: 1,
  scrollTrigger: {
    trigger: ".cards",
    scroller: "body",
    start: "top 70%",
    end: "top 65%",
    scrub: 1,
  },
});

// === GSAP quotes ===
gsap.from("#colon1", {
  y: -70,
  x: -70,
  scrollTrigger: {
    trigger: "#colon1",
    scroller: "body",
    start: "top 55%",
    end: "top 45%",
    scrub: 4,
  },
});

gsap.from("#colon2", {
  y: 70,
  x: 70,
  scrollTrigger: {
    trigger: "#colon1",
    scroller: "body",
    start: "top 55%",
    end: "top 45%",
    scrub: 4,
  },
});

// === GSAP page4 heading ===
gsap.from("#page4 h1", {
  y: 50,
  opacity: 0,
  duration: 1,
  scrollTrigger: {
    trigger: "#page4",
    scroller: "body",
    start: "top 75%",
    end: "top 50%",
    scrub: 2,
  },
});
